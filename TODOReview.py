...
class Engine():
	...
	def files(self):
		seen_paths = set()
		exclude_patterns = self.exclude_folders + self.exclude_files
		...
		for filepath in self.filepaths:
			...
			if p in seen_paths or any(p.search(filepath) for p in exclude_patterns):
				continue
			seen_paths.add(p)
			yield p
# TODO: Fix bug
...
class TodoReviewCommand(sublime_plugin.TextCommand):
	...
	def render(self, results, time, count):
		view = self.get_view()
		view.run_command('todo_review_render', {
			"results": results,
			"time": time,
			"count": count,
			"args": self.args
...
class TodoReviewRender(sublime_plugin.TextCommand):
	...
	def draw_header(self):
		...
		res += '\n'
		deb = self.rview.insert(self.edit, self.rview.size(), res)

	def draw_results(self):
		data = [x[:] for x in [[]] * 2]
		for patt, items in self.sorted:
			items = list(items)
			res = ''.join(['\n## ', patt.upper(), ' (', str(len(items)), ')\n'])
		...
		d = dict(('{},{}'.format(k.a, k.b), v) for k, v in zip(data[0], data[1]))
